...
export async function saveDocumentVault(docData) {
  ...
  for (const [docKey, docVal] of Object.entries(documents)) {
    ...
    let secureUrl = docVal.dataUrl || docVal.url;
    let publicId = `doc_${user_id}_${docKey}`;

    // Upload to Cloudinary if base64 dataUrl is provided
    if (docVal.dataUrl && typeof docVal.dataUrl === "string") {
      try {
    ...
    docEntries.push({
      ...
      public_id: publicId,
      data_url: docVal.dataUrl || secureUrl, // Kept for offline/high-res instant viewer
  ...
  try {
    ...
    console.warn("Mongoose DocumentVault save note:", err.message);
  }
...
export async function getDocumentVault(userId) {
  try {
    ...
    console.warn("Mongoose DocumentVault get note:", err.message);
  }
...
export async function saveBiometricFaceLog(bioData) {
  ...
  if (face_snapshot && typeof face_snapshot === "string") {
    try {
      const cloudUpload = await uploadToCloudinary(
        face_snapshot,
  ...
  const recordPayload = {
    ...
    face_snapshot_base64: face_snapshot,
  ...
  try {
    ...
    console.warn("Mongoose BiometricFaceLog save note:", err.message);
  }
