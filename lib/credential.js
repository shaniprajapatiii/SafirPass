...
export function attributeValue(kyc, key) {
  ...
  switch (key) {
    ...
    case "validity":
      return kyc.entry_date && kyc.exit_date
        ? `${kyc.entry_date} → ${kyc.exit_date}`
        : "—";
    case "emergency_contact":
      return kyc.emergency_contact || "—";
    default:
      return "—";
  }
}
...
export function barcodeValue(kyc) {
  const base = (kyc?.tourist_id || "").replace(/[^A-Z0-9]/gi, "");
  if (!base) return "0000000000";
  const check = Array.from(kyc?.id || "id").reduce(
